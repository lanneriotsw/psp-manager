...
#
# This file is copyright under the latest version of the MIT.
...
INSTALL_LOG_LOC="/opt/lanner/psp-manager/install.log"
# This is a file used for the colorized output
coltable="/opt/lanner/psp-manager/COL_TABLE"
...
BASE_URL="https://link.lannerinc.com"
PM_GIT_URL="https://github.com/lanneriotsw/psp-manager.git"
PM_LOCAL_REPO="/opt/lanner/psp-manager"
PSP_INSTALL_DIR="/opt/lanner/psp"
DOWNLOAD_DIR="/opt/lanner/download"
...
check_input_param() {
    ...
    printf "  %b Input parameter check" "${INFO}"

    if [[ "${product_type}" && "${version_name}" ]]; then
        printf "%b  %b Input parameter check\\n" "${OVER}" "${TICK}"
    else
        printf "%b  %b Input parameter check\\n" "${OVER}" "${CROSS}"
        printf "  %b Usage:\\n" "${INFO}"
...
check_file_for_download() {
    ...
    printf "  %b PSP file check" "${INFO}"

    if curl --output /dev/null --silent --head --fail ${BASE_URL}/psp/PSP2.x/${product_type}/${file_name}; then
        printf "%b  %b PSP file check\\n" "${OVER}" "${TICK}"
    else
        printf "%b  %b PSP file check\\n" "${OVER}" "${CROSS}"
        printf "  %b PSP file not found, see ${BASE_URL}/psp for support%b\\n" "${INFO}" "${COL_NC}"
        exit 1
    fi
}
...
install_psp() {
    ...
    if [ ! -f "${DOWNLOAD_DIR}/${file_name}" ]; then
        ...
        curl -L ${BASE_URL}/psp/PSP2.x/${product_type}/${file_name} -o ${DOWNLOAD_DIR}/${file_name}
    fi
    ...
    # TODO: load driver manually
    # cd ${PSP_INSTALL_DIR}/bin/amd64
    # . loaddriver.sh
    # cd ${PSP_INSTALL_DIR}

    # LEC-7242
    if [ ${product_type} = "LEC-7242" ]; then
        local config_tool_file="config-tool-20201229.tar.bz2"
        
        if [ ! -f "${DOWNLOAD_DIR}/${config_tool_file}" ]; then
            printf "\\n  %b Download ${config_tool_file}\\n" "${INFO}"
            curl -L ${BASE_URL}/psp/PSP2.x/LEC-7242/tool/${config_tool_file} -o ${DOWNLOAD_DIR}/${config_tool_file}
        fi

        if [ ! -d ${PSP_INSTALL_DIR}/tool ]; then
            printf "\\n  %b Install ${config_tool_file}\\n" "${INFO}"
            tar jxf ${DOWNLOAD_DIR}/${config_tool_file} -C ${PSP_INSTALL_DIR}
            make -C ${PSP_INSTALL_DIR}/tool
        fi
    fi

    chmod 755 ${PM_LOCAL_REPO}/load-driver.sh
...
main() {
    ...
    check_input_param "$1" "$2"

    check_file_for_download "$1" "$2"
    ...
    install_psp "$1" "$2" | tee -a /proc/$$/fd/3
